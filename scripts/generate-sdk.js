...
// Only generate the list of operations for the README
const readMeMode = mode === 'readme';

const output = readMeMode ? [] : [
  '/**',
...
for (const [path, pathDefinition] of Object.entries(openApiDoc.paths)) {
  ...
  for (const [method, operationDefinition] of Object.entries(pathDefinition)) {
    ...
    if (readMeMode) {
      output.push(`${operationName}(${parameters.map(p => p.split(':')[0]).join(', ')})`);
      continue;
    }
